...
constexpr auto parse_replacement_field_then_tail(S fmt) {
    ...
    if constexpr (c == '}') {
        ...
        if constexpr (result.end >= str.size() || str[result.end] != '}') {
            ...
            return parse_tail<Args, result.end + 1, result.next_arg_id>(spec_field<char_type, typename field_type<T>::type, ARG_INDEX>{result.fmt}, fmt);
        }
    }
}
    ...
    if constexpr (str[POS] == '{') {
        ...
        if constexpr (str[POS + 1] == '{') {
            ...
            if constexpr (arg_id_result.kind == arg_id_kind::index) {
                ...
                return parse_replacement_field_then_tail<get_type<arg_index, Args>, Args, arg_id_end_pos, arg_index, manual_indexing_id>(fmt);
            } else if constexpr (arg_id_result.kind == arg_id_kind::name) {
                constexpr auto arg_index = get_arg_index_by_name(arg_id_result.arg_id.name, Args{});
                if constexpr (arg_index >= 0) {
                    constexpr auto next_id = ID != manual_indexing_id ? ID + 1 : manual_indexing_id;
                    return parse_replacement_field_then_tail<decltype(get_type<arg_index, Args>::value), Args, arg_id_end_pos, arg_index, next_id>(fmt);
                } else if constexpr (c == '}') {
