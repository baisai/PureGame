...
template <typename... T>
inline auto current_zone(T...) -> time_zone* {
    return nullptr;
}
...
auto write_encoded_tm_str(OutputIt out, string_view in, const std::locale& loc) -> OutputIt {
    if (const_check(detail::use_utf8) && loc != get_classic_locale()) {
        ...
        if (!u.convert({unit.buf, to_unsigned(unit.end - unit.buf)})) FMT_THROW(format_error("failed to format time"));
...
struct is_same_arithmetic_type : public std::integral_constant<bool, (std::is_integral<Rep1>::value && std::is_integral<Rep2>::value) ||
                                                                         (std::is_floating_point<Rep1>::value && std::is_floating_point<Rep2>::value)> {};

FMT_NORETURN inline void throw_duration_error() { FMT_THROW(format_error("cannot format duration")); }
...
auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To {
    ...
    if (const_check(factor::num != 1)) {
        ...
        if (const_check(!std::is_unsigned<common_rep>::value) && count < min) throw_duration_error();
        count *= factor::num;
    }
...
#if FMT_USE_LOCAL_TIME
template <typename Duration, FMT_ENABLE_IF(detail::has_current_zone<Duration>())>
...
FMT_CONSTEXPR inline auto get_units() -> const char* {
    ...
    if (std::is_same<Period, std::micro>::value) return detail::use_utf8 ? "µs" : "us";
...
void write_fractional_seconds(OutputIt& out, Duration d, int precision = -1) {
    ...
    using subsecond_precision =
        std::chrono::duration<typename std::common_type<typename Duration::rep, std::chrono::seconds::rep>::type, std::ratio<1, pow10(num_fractional_digits)>>;
...
void write_floating_seconds(memory_buffer& buf, Duration duration, int num_fractional_digits = -1) {
    ...
    fmt::format_to(std::back_inserter(buf), FMT_STRING("{:.{}f}"),
                   std::fmod(val * static_cast<rep>(Duration::period::num) / static_cast<rep>(Duration::period::den), static_cast<rep>(60)),
...
public:
    ...
    void on_year(numeric_system ns, pad_type pad) {
        if (is_classic_ || ns == numeric_system::standard) return write_year(tm_year(), pad);
    ...
    void on_dec_month(numeric_system ns, pad_type pad) {
        if (is_classic_ || ns == numeric_system::standard) return write2(tm_mon() + 1, pad);
        format_localized('m', 'O');
    }

    void on_dec0_week_of_year(numeric_system ns, pad_type pad) {
        if (is_classic_ || ns == numeric_system::standard) return write2((tm_yday() + days_per_week - tm_wday()) / days_per_week, pad);
        format_localized('U', 'O');
    }
    ...
    void on_iso_week_of_year(numeric_system ns, pad_type pad) {
        if (is_classic_ || ns == numeric_system::standard) return write2(tm_iso_week_of_year(), pad);
...
struct chrono_format_checker : null_chrono_spec_handler<chrono_format_checker> {
    ...
    FMT_CONSTEXPR void on_duration_value() const {
        if (has_precision_integral) FMT_THROW(format_error("precision not allowed for this argument type"));
    }
...
inline auto get_milliseconds(std::chrono::duration<Rep, Period> d) -> std::chrono::duration<Rep, std::milli> {
    ...
    const auto d_as_whole_seconds = detail::duration_cast<std::chrono::seconds>(d_as_common);
    // this conversion should be nonproblematic
    const auto diff = d_as_common - d_as_whole_seconds;
    const auto ms = detail::duration_cast<std::chrono::duration<Rep, std::milli>>(diff);
    return ms;
#else
...
struct chrono_formatter {
    ...
    void on_text(const char_type* begin, const char_type* end) { copy<char_type>(begin, end, out); }
...
public:
    weekday() = default;
    constexpr explicit weekday(unsigned wd) noexcept : value_(static_cast<unsigned char>(wd != 7 ? wd : 0)) {}
...
public:
    ...
    constexpr year_month_day(const year& y, const month& m, const day& d) noexcept : year_(y), month_(m), day_(d) {}
...
struct formatter<sys_time<Duration>, Char> : formatter<std::tm, Char> {
    FMT_CONSTEXPR formatter() { this->fmt_ = detail::string_literal<Char, '%', 'F', ' ', '%', 'T'>(); }
...
struct formatter<local_time<Duration>, Char> : formatter<std::tm, Char> {
    ...
    auto format(local_time<Duration> val, FormatContext& ctx) const -> decltype(ctx.out()) {
        ...
        auto subsecs = detail::duration_cast<Duration>(epoch - detail::duration_cast<std::chrono::seconds>(epoch));
